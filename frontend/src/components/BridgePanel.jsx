...
const STAGE_LABELS = {
  vision:  { label: "Visual Analysis",     color: "teal",   icon: "👁" },
  context: { label: "Historical Context",  color: "blue",   icon: "📚" },
  risk:    { label: "Risk Assessment",     color: "purple", icon: "⚖" },
};

const HEADING_LABELS = {
  "0": "N", "45": "NE", "60": "NE", "90": "E", "120": "SE",
  "135": "SE", "180": "S", "225": "SW", "240": "SW",
  "270": "W", "300": "NW", "315": "NW",
};

const STAGE_COLORS = {
  teal:   { border: "border-teal-200",   bg: "bg-teal-50",   dot: "bg-teal-500",   text: "text-teal-700",   badge: "bg-teal-100 text-teal-700" },
  blue:   { border: "border-blue-200",   bg: "bg-blue-50",   dot: "bg-blue-500",   text: "text-blue-700",   badge: "bg-blue-100 text-blue-700" },
  purple: { border: "border-purple-200", bg: "bg-purple-50", dot: "bg-purple-500", text: "text-purple-700", badge: "bg-purple-100 text-purple-700" },
};

const THINKING_STEP_STYLE = `
@keyframes thinkStepIn {
  from { opacity: 0; transform: translateY(4px); }
  to   { opacity: 1; transform: translateY(0); }
}
.think-step-in {
  animation: thinkStepIn 0.22s ease forwards;
}
`;
  ...
  if (blocks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-8 gap-3">
        ...
        <p className="text-xs text-gray-400 italic">Fetching Street View imagery…</p>
      </div>
    );
  }

  // Flatten to a global step index so every step gets a unique key that
  // never changes once rendered — this lets the browser keep the animation
  // for already-shown steps while new ones fade in.
  let globalIdx = 0;

  return (
    <>
      <style>{THINKING_STEP_STYLE}</style>
      <div className="space-y-3 px-4 py-3">
        {blocks.map((block, bi) => {
          const meta = STAGE_LABELS[block.stage] || { label: block.stage, color: "blue", icon: "•" };
          const c = STAGE_COLORS[meta.color];
          const headingStr = block.heading
            ? ` · ${HEADING_LABELS[block.heading] ?? block.heading + "°"}`
            : "";
          return (
            <div key={`${block.stage}-${block.heading ?? "none"}-${bi}`}
              className={`rounded-lg border ${c.border} ${c.bg} p-3`}
            >
              <div className="flex items-center gap-1.5 mb-2">
                <span className="text-sm">{meta.icon}</span>
                <span className={`text-xs font-semibold ${c.text}`}>{meta.label}{headingStr}</span>
              </div>
              <ol className="space-y-1.5">
                {block.steps.map((step, si) => {
                  const key = globalIdx++;
                  return (
                    <li key={key} className="think-step-in flex gap-2 items-start">
                      <span className={`flex-shrink-0 w-4 h-4 rounded-full ${c.dot} text-white text-xs flex items-center justify-center font-bold mt-0.5`}>
                        {si + 1}
                      </span>
...
function BridgePreAnalysis({ bridge }) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {isAnalyzing ? (
        /* Live thinking feed while analysing */
        <div className="flex flex-col h-full overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-2.5 border-b border-gray-100 bg-gray-50 flex-shrink-0">
            <div className="w-3.5 h-3.5 border-2 border-gray-300 border-t-blue-500 rounded-full animate-spin flex-shrink-0" />
            <p className="text-xs font-semibold text-gray-600">AI is analysing this bridge…</p>
          </div>
          <div className="flex-1 overflow-y-auto">
            <ThinkingFeed osm_id={bridge.osm_id} />
          </div>
        </div>
      ) : (
        /* Normal pre-analysis metadata + button */
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-4 py-5">
            <button
              ...
              className="w-full py-3 px-4 rounded-lg font-semibold text-sm transition-colors bg-blue-600 hover:bg-blue-700 text-white"
            >
              Run Deep Analysis
            </button>
